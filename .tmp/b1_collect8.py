...
OUTLOG = os.path.join(ROOT, ".tmp", "dgt_b1_v8.log")
...
def main() -> int:
    fo = open(os.path.join(ROOT, ".tmp", "b1_v8.out"), "wb")
    ...
    step("START", lambda: b.tap(START, 6, 2.0))
    step("A×3", lambda: (b.tap(A, 4, 1.2), b.tap(A, 4, 1.2), b.tap(A, 4, 1.5)))
    step("DOWN 长按", lambda: b.hold(DOWN, 25, 1.0))
    step("RIGHT 长按", lambda: b.hold(RIGHT, 25, 1.0))
    step("UP 长按", lambda: b.hold(UP, 25, 1.0))
    step("LEFT 长按", lambda: b.hold(LEFT, 25, 1.0))
    step("对话 A×4", lambda: [b.tap(A, 4, 1.0) for _ in range(4)])
    step("START 开菜单", lambda: b.tap(START, 6, 1.5))
    step("DOWN×2", lambda: (b.tap(DOWN, 5, 0.8), b.tap(DOWN, 5, 0.8)))
    step("A 确认", lambda: b.tap(A, 5, 1.5))
    step("B B 返回", lambda: (b.tap(Bk, 5, 0.8), b.tap(Bk, 5, 1.2)))
    for i in range(3):
        step(f"补走 {i+1}", lambda: (b.hold(DOWN, 18, 0.6), b.hold(RIGHT, 18, 0.6),
                                     b.tap(A, 4, 1.2), b.hold(UP, 18, 0.6)))
    step("START→DOWN→A", lambda: (b.tap(START, 6, 1.0), b.tap(DOWN, 5, 0.8),
                                  b.tap(A, 5, 2.0)))
    step("B×2", lambda: (b.tap(Bk, 5, 1.0), b.tap(Bk, 5, 1.0)))
