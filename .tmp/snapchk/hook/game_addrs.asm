...
; 字模缓存配置（唯一入口，全 ROM 仅 2 个调用点 0x080685CA / 0x0806F006）：
;   r0 = win, r1 = tileBase。
;   写 *0x0300032E=0、*0x03000328=*(u32*)win(活跃模板单例)、*0x0300032C=tileBase、
;   win[+0x16]=tileBase；返回「本 font 需要的号数」（512 / 256）。
;   ⇒ 引擎字模写 `tileData + (tileBase+index)*32`（font0/3 为 `+tileBase*32+index*64`），
;     index 遍历 [0,256)；实测全窗口 tpl[+8]==3 ⇒ 512 号 ⇒ **占满整个 charBlock + 溢出 1 号**。
SetWindowTileCache                     equ 0x08002950  ; C: ADDR_SET_WINDOW_TILE_CACHE
...
; DrawGlyphTiles = 美版 0x08006874 同构（2026-09-12 结构指纹实证，非 pokeruby_jp.sym
;   里那个错的 0x08005178）。结构：GetGlyphTilePointers → fontNum 跳转表分派 →
;   两次 DrawGlyphTile_*（第二次 +32 = 下半砖）。被 FontFunc tm0/tm1/tm2/tm3 处理器调用。
;   ★ Wokann 方案唯一钩点 = 本函数 +2（bl DrawGlyphTilesChinese）。
;   ★ 注意：日版 Window 布局与美版不同（日版 +0x10=text/+0x20=tileData；
;     美版 +0x20=*text/+0x24=tileData）⇒ pokeRS 的 DrawGlyphTilesChinese.s 不能照抄。
DrawGlyphTiles                         equ 0x08003630  ; C: ADDR_DRAW_GLYPH_TILES
