...
- **🔴 铁律：任何代码改动前，「预计效果」必须能翻译成一张实机截图的预期样子**。看不到这个预期不写代码。改动完**看到截图符合预期**才算交付，看不到就是没做完（2026-09-03 用户纠偏）。
- **🔴 铁律：执行流程固定四步，禁止跳过**——①静态分析（反汇编/查表）②动态结合（gdb_patcher 采集/字段追踪）③实操验证（写代码+实机截图对比，**只有截图符合预期才算**）④总结结论（这时才能写进 MEMORY）。**禁止用未经验证的静态结论去推进下一步**，禁止「我猜是这样」当真理（2026-09-03 用户纠偏，原因是今晚我把没经过实机验证的 tm0/tm1 静态结论当成已证事实反复推进，把项目节奏打乱了）。
- **场景门控边界（2026-09-04 v8 定稿，取代 v6）**：❌ 逐窗登记/声明式配置表(kTm1Windows)/atlas 扫描/OBJ 运行时避让/静态行带表(row_tab)/off 分区——统统否掉。✅ tile 号 = **唯一顺序分配器 v8_alloc_tile(win,font_px,glyph_len)**（运行时扫 tilemap 活引用得避让带，顺序放入、跳过占用、领连续 glyph_len 空闲），屏幕位置继续交官方光标。✅ 字号 = `getFontSize(win)` 钩子（font4/tm2→8px，设置菜单 curX<8→16 否则12，其余12）。✅ 12px 相位 = 按行隔离单变量（`ADDR_V8_PHASE`+行标识`tpl^curY`），非全局 8 槽表。**tile 号分配 与 相位 px 正交**：前者是顺序分配器，后者是行内像素游标。
- **v8 已知边界（2026-09-04 实机截图实证）**：①队伍页 HP 条上方 Pokemon 状态图标被中文覆盖（需扩避让带或该区域 fallback 只画官方日文）；②设置菜单偶发缺角（相位共享 + 动态领号在「字符短+边界 tile 紧邻」组合下溢出，治本=glyph_len 加安全余量）；③**根治来回切换残留 BUG 的根本**=所有跨窗口状态（游标/相位/last_tile/行标识）在 InitTextPrinter 边界或 tpl 变化时复位，不依赖任何「行指纹 key 续接」启发式。
...
- **🔴 场景规则表·精细落址（2026-09-01 落地，参考 kOptWindow 轻量化）**：条件分字号 + 每行固定基址。`struct V6Zone{cx_hi,font_px}` + `struct V6SceneRule{tpl,row_y0,row_shift,row_tab,row_n,zones,zone_n}`，查表 `v6_scene_lookup/font/row_base`。命中规则 → 字号由 curX 分区定(key 16/value 12)、tile=行基址+(px>>3)*2 确定性排列重绘幂等；未命中 → 全局高水位 v6_alloc_tile 回退。**边界=分区独立**(16px 区前/12px 区后,不任意混排;任意混排需三段式)。16px key 走整格 2 列 tile=行基址+(px>>3)*2+col*2、px+=16。样板=设置菜单 0x081BB874。行基址=人为划固定区间(赌官方不引用)=方案 A1 降级版。文档 docs/V6_场景规则表_精细落址.md。
...
- 打包：hook build.bat→根 build.bat→check_rom_hook。v6 game.bin 7700B（2026-09-01 场景规则表落地后），.data 0 / .bss 0。
