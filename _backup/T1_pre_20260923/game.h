 ...
 * 12px 相位自存（R1-A 起 = 单 px 游标，无行标识检测）。
 */

/* ---- 宽度单源（R1-A 2026-09-23）：步进/墨宽唯一定义处 ----
 * C 侧（resolve_draw / chs_print_px / GetStringWidth）与离线折行
 * src/meowth/text_wrap.py（正则解析本文件）都从本组取值；
 * 调用点禁止再写 12/11/10/9 宽度字面量。 */
#define CHS_GLYPH_ADVANCE_PX       12   /* 大库 11×11 步进 */
#define CHS_GLYPH_INK_PX           11   /* 大库墨宽（resolve_draw ink，不含阴影） */
#define CHS_GLYPH_ADVANCE_SMALL_PX 10   /* 小库 9×9 / Middle 9×11 步进（tm2/fn4/场景表） */
#define CHS_GLYPH_INK_SMALL_PX      9   /* 两小档墨宽 */
