 ...
 *   include/text_scene.h  + src/text/text_scene.c  = **数据**：本文件的结构体
 *                          定义 + 每个窗口一条的静态配置（kOptWindow 等）。
 *   include/text_layout.h + src/text/text_layout.c = **算法**：查表、求值、
 *                          分区选择、槽查询、落址（scene_gctn_* 等）。
 ...
 * ⚠ text_scene.c = 纯数据文件，一个函数都没有。查询/落址算法在 text_layout.c，
 *   接口见 text_layout.h（本文件不重复声明）。 */
