 ...
 *
 * 每行 32 tile（候选 A 12 + B 10 + C 10，见 kOptZones；三档足迹互不重叠）。
...
static const struct Tm1Zone kOptZones[] = {
    ...
    /* ③ 候选 B curX 19..21：DYN 12px。gdb [CFF] 实测这一档的中文只有
     *    "普通"（@19，2 字 = 6 tile）；LR / L / 7 都走原生，不吃中文区。 */
    { 22u,   TM1_ZONE_DYN, 0u, 12u, 10u },

    /* ④ 候选 C curX>=22：DYN 12px。⚠ gdb [CFF] 实测：**打到底 / 立体声（3 字）
     *    在这一档（cx=22）**，不是 ③！3 字 12px 占 10 tile，off 必须 ≤ 22：
     *    足迹 [22,32) 恰好收在本行 32 tile 内。
     *    之前 off=24/span=8 是按"这里只有'快'1 字"设计的 → 足迹 [24,34)
     *    越过行界，尾部 2 格压住下一行候选 A 区（对战规则行的"打到底"踩住
     *    声音行"单声道"的首字 tile）→ 表现为行尾残留"单"、单声道首字随
     *    重绘在碎片/恢复间切换。 */
    { 0xFFu, TM1_ZONE_DYN, 0u, 22u, 10u },
};
