    ...
    if not reg_hits and not slots:
        ...
        if hint_trap and in_bios(pc):
            print(
                ...
                f"  python src/util/gdb_patcher.py trap 0x{v:08X}"
            )
        elif hint_trap:
            print(
...
def run_trap(
    ...
    *,
    ...
    print(
        ...
        f"  python src/util/gdb_patcher.py romscan 0x{value & 0xFFFFFFFF:08X}"
    )
    with GdbClient(host, port) as gdb:
        regs0 = gdb.read_regs()
        pc0 = regs0.get("r15", 0)
            ...
            while True:
                ...
                if pc == (SOFTRESET_BP & ~1):
                    ...
                    print(
                        ...
                        f"请跑: python src/util/gdb_patcher.py romscan 0x{value & 0xFFFFFFFF:08X}"
                    )
                    return 0
                ...
                if which is None:
                    if in_bios(pc):
                        ...
                        print(
                            f"请改用: python src/util/gdb_patcher.py romscan "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        if first == "trap":
            if not args.value:
                print(
                    "trap 需要坏地址，例如: python src/util/gdb_patcher.py trap 0x5F0A00F9",
                    file=sys.stderr,
                )
                return 2
